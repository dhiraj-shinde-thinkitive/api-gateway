...
public class UsageTrackingService {
     ...
     */
    @Async
    public void logUsageAsync(ServerWebExchange exchange, ApiKeyService.ApiKeyMetadata metadata, Integer responseStatus, Long responseTimeMs, String errorMessage) {
    ...
    private UsageLogMessage createUsageLogMessage(ServerWebExchange exchange,
        ...
        String apiKeyUsed = extractApiKey(exchange);

        return new UsageLogMessage(
            metadata != null ? Long.valueOf(metadata.apiKeyId()) : null,
            ...
            correlationId,
            LocalDateTime.now(),
            apiKeyUsed != null ? apiKeyUsed.substring(0, Math.min(8, apiKeyUsed.length())) + "..." : null
        );
    }
